...
let zJS = {},
	...
	// UTILS
	drawBounds, setZeroTimeout, typoGraph, origins, checkUAEngine, hasMathMLSupport, isBrowserAPI,
	// MATH
	gcd, factorial, isFib, fibTo, fib, primes, swap, isPowOf2, isMinus0, toBinary, countSort, shuffle;
...
/*
for (let i = arr.length - 1, j; i > 0; i -= 1) {
    j = ~~(Math.random() * i);
    arr[i] ^= arr[j];
    arr[j] ^= arr[i];
    arr[i] ^= arr[j];

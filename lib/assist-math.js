...
/**
 *	Get prime numbers from interval.
 *	@param {Number} m - Left bound of the range.
 *	@param {Number} [n] - Right bound of the range.
 *	@returns {Object[]} Array of prime numbers.
 */
findPrimes = (m, n) => {
	if (!arguments.length || arguments.length > 2 || ![...arguments].every(isInt)) {
		throw new CallError(ERRORS.math.primes);
	}
	...
	let range = Array.from(Array(n + 1), (v, i) => i),
		ret = [],
		i = 1;
	...
	while (i++ <= n) {
		if (range[i]) {
			ret.push(range[i]);
			for (let j = i; j <= n; j += i) range[j] = 0;
		}
	}
	
	let l = ret.length; i = -1;
	
	while (i++ < l) {
		if (ret[i] >= m) return ret.slice(i);
	}
...
math = {
	...
	findPrimes: findPrimes, // Get prime numbers from interval.
